...
class BusinessIntelligenceService {
    ...
    async getPerformanceMetrics(studioId, startDate, endDate) {
        const [kpis, conversionFunnel, servicePerformance, seasonalTrends,] = await Promise.all([
    ...
    async getPredictiveAnalytics(studioId) {
        const [revenueForeccast, demandForecast, churnRisk, pricingRecommendations,] = await Promise.all([
    ...
    async getRevenueMetrics(studioId, currentStart, currentEnd, previousStart, previousEnd) {
    ...
    async getEquipmentMetrics(studioId) {
        const [total, statusCounts, valueData, maintenanceNeeded,] = await Promise.all([
    ...
    async calculateKPIs(studioId, startDate, endDate) {
        ...
        const [monthlyRevenue, bookingConversion, avgBookingValue, clientRetention, teamUtilization,] = await Promise.all([
            this.db.payment.aggregate({
                where: {
                    studioId,
                    status: 'COMPLETED',
