...
class EquipmentService {
    ...
    async createEquipment(data, userId, imageFile) {
        ...
        let imageUrl;
        if (imageFile) {
            const uploadResult = await this.fileService.uploadFile(imageFile, 'equipment', data.studio.connect?.id);
    ...
    async checkOutEquipment(equipmentId, userId, bookingId, expectedReturnAt, notes) {
        ...
        const reminderDate = (0, dayjs_1.default)(expectedReturnAt).subtract(1, 'hour').toDate();
    ...
    async checkInEquipment(assignmentId, condition, notes, damageReported = false, damageDescription) {
        ...
        await this.db.equipment.update({
            ...
            data: {
                ...
                totalHoursUsed: {
                    increment: (0, dayjs_1.default)().diff(assignment.checkedOutAt, 'hours', true),
                },
            },
        });
        if (damageReported) {
    ...
    async checkEquipmentAvailability(equipmentIds, startDate, endDate, excludeBookingId) {
        const availability = await Promise.all(equipmentIds.map(async (equipmentId) => {
    ...
    async logMaintenance(equipmentId, data, userId) {
        ...
        if (data.nextDueDate) {
            const reminderDate = (0, dayjs_1.default)(data.nextDueDate).subtract(7, 'days').toDate();
    ...
    async getMaintenanceSchedule(studioId, daysAhead = 30) {
        ...
        return equipment.map((eq) => {
            ...
            const daysUntilDue = (0, dayjs_1.default)(nextDue).diff((0, dayjs_1.default)(), 'days');
            return {
                equipmentId: eq.id,
                equipment: eq,
    ...
    async getEquipmentStats(studioId) {
        const [totalEquipment, statusCounts, categoryBreakdown, valueData, utilizationData, maintenanceData, ageData,] = await Promise.all([
    ...
    calculateDepreciation(equipment) {
        ...
        const yearsOwned = (0, dayjs_1.default)().diff(equipment.purchaseDate, 'years', true);
        ...
        const currentValue = Math.max(Number(equipment.purchasePrice) - totalDepreciation, Number(equipment.purchasePrice) * 0.1);
        return {
