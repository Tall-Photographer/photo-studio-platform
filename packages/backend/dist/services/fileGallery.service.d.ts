...
export declare class FileGalleryService {
    ...
    uploadFile(file: Express.Multer.File, options: FileUploadOptions, uploadedBy: string): Promise<File>;
    batchUploadFiles(files: Express.Multer.File[], options: FileUploadOptions, uploadedBy: string): Promise<File[]>;
    getGallery(projectId: string, options?: GalleryOptions): Promise<Gallery>;
    createShareableGallery(projectId: string, options: {
        name?: string;
        description?: string;
        password?: string;
        expiresIn?: number;
        ...
        watermarkEnabled?: boolean;
        fileIds?: string[];
    }, createdBy: string): Promise<Gallery>;
    ...
    batchUpdateFiles(fileIds: string[], updates: {
        isSelected?: boolean;
        isEdited?: boolean;
        tags?: string[];
    }, userId: string): Promise<number>;
    deleteFile(fileId: string, userId: string): Promise<void>;
    createDeliveryPackage(projectId: string, options: {
        name: string;
        fileIds: string[];
        deliveryMethod: 'download' | 'usb' | 'cloud';
        expiresIn?: number;
        notifyClient?: boolean;
    }, createdBy: string): Promise<DeliveryPackage>;
    processImage(fileId: string, options: FileProcessingOptions, processedBy: string): Promise<File>;
    ...
    handleClientSelection(galleryId: string, fileIds: string[], clientInfo: {
        name: string;
        email: string;
        notes?: string;
    }): Promise<void>;
