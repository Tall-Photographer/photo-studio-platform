...
class ClientService {
    ...
    async getClients(studioId, filters, page = 1, limit = 20, sortBy = 'createdAt', sortOrder = 'desc') {
        const where = {
            studioId,
            deletedAt: null,
        };
    ...
    async updateClient(clientId, studioId, data, userId) {
        ...
        await database_service_1.DatabaseService.getInstance().updateClientStatistics(clientId);
        await this.auditService.log({
            studioId,
            userId,
    ...
    async mergeClients(primaryClientId, secondaryClientId, studioId, userId) {
        ...
        const result = await this.db.$transaction(async (tx) => {
            ...
            const updated = await tx.client.update({
                ...
                data: {
                    ...
                    discountPercentage: Math.max(Number(primaryClient.discountPercentage), Number(secondaryClient.discountPercentage)),
        ...
        await database_service_1.DatabaseService.getInstance().updateClientStatistics(primaryClientId);
        await this.auditService.log({
            studioId,
            userId,
    ...
    async calculateLoyaltyRewards(clientId, studioId) {
        ...
        const availableRewards = rewards.filter((reward) => client.loyaltyPoints >= reward.pointsRequired);
        return {
            points: client.loyaltyPoints,
            nextRewardPoints: rewards.find((r) => r.pointsRequired > client.loyaltyPoints)?.pointsRequired,
            availableRewards,
        };
    }
    ...
    async getClientInsights(studioId) {
        ...
        const [totalClients, activeClients, vipClients, revenueData, clientsBySource, monthlyData,] = await Promise.all([
