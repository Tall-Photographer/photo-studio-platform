...
class IntegrationService {
      ...
      const encryptedTokens = await this.cryptoService.encrypt(
        JSON.stringify({
          ...
          expiresAt: (0, dayjs_1.default)()
            ...
            .toISOString(),
          refreshExpiresAt: (0, dayjs_1.default)()
            .add(x_refresh_token_expires_in, "seconds")
            .toISOString(),
        }),
      );
      const companyResponse = await axios_1.default.get(
  ...
  async getGoogleOAuth2Client(userId) {
    ...
    if (tokens.expiry_date && tokens.expiry_date < Date.now()) {
      ...
      const newEncryptedTokens = await this.cryptoService.encrypt(
  ...
  async getQuickBooksAccessToken(studioId) {
    ...
    const tokens = JSON.parse(tokensJson);
    if (
      (0, dayjs_1.default)(tokens.expiresAt).isBefore((0, dayjs_1.default)())
    ) {
      const config = this.integrationConfigs.get("quickbooks")?.quickbooks;
  ...
  async syncCustomerToQuickBooks(client, studioId, accessToken) {
    const integration = await this.getQuickBooksIntegration(studioId);
    if (!integration) throw new Error("QuickBooks not connected");
    const searchResponse = await axios_1.default.get(
    ...
    const createResponse = await axios_1.default.post(
