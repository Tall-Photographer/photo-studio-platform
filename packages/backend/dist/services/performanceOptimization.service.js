...
class PerformanceOptimizationService {
    ...
    setupPerformanceMonitoring() {
        ...
        this.performanceQueue.add('performance-check', {}, {
            repeat: {
                every: 5 * 60 * 1000,
            },
        });
        ...
        const frequentQueries = [
            ...
            async () => {
                const bookings = await this.db.booking.findMany({
                    where: {
                        startDateTime: { gte: (0, dayjs_1.default)().subtract(30, 'days').toDate() },
                    },
    ...
    async cleanupOldData() {
        const [sessions, logs, notifications] = await Promise.all([
            ...
            this.db.auditLog.deleteMany({
                where: {
                    createdAt: { lt: (0, dayjs_1.default)().subtract(90, 'days').toDate() },
                },
            }),
            this.db.notification.deleteMany({
                where: {
                    isRead: true,
                    createdAt: { lt: (0, dayjs_1.default)().subtract(30, 'days').toDate() },
                },
            }),
        ]);
        return {
