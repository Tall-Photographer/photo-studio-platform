...
class EmailMarketingService {
    ...
    async sendTestEmail(campaignId, studioId, testEmails, userId) {
        ...
        for (const email of testEmails) {
            const personalizedContent = await this.personalizeContent(campaign.htmlContent, sampleClient || {
                firstName: 'Test',
                lastName: 'User',
                email,
    ...
    async sendCampaign(campaignId, studioId, userId) {
        ...
        const recipients = await this.getAudienceClients(studioId, campaign.audienceFilter);
        ...
        for (let i = 0; i < recipients.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async (client) => {
                try {
                    const personalizedContent = await this.personalizeContent(campaign.htmlContent, client, campaign);
    ...
    async getCampaignInsights(studioId, period = 30) {
        ...
        const [totalCampaigns, totalSent, avgOpenRate, avgClickRate, topPerformingCampaigns, engagementByDay, engagementByHour,] = await Promise.all([
